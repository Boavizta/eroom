...
Ce script ne fait AUCUN calcul de score : le template produit est vierge
...
#   🚫 Non applicable                    -> 0  (exclu, compte quand même dans le poids total)
...
# piliers, confirmé par le texte de l'onglet "À LIRE" (Étape 6). Mapping
...
def build_pillar(json_dir, tab_name, title):
    ...
    out = [f"## {title}\n"]
    # ligne 1 (index 1 dans les rows) porte la description courte du pilier
    if len(rows) > 1:
        desc = cell(rows[1], 2)
        if desc:
            out.append(f"*{desc}*\n")
...
def build_synthese(json_dir, criteria_counts):
    ...
    out.append(
        ...
        "les 6 piliers ci-dessous (hors Diagnostic rapide, qui les recouvre en "
    ...
    out.append(
        ...
        "rendu (scores fictifs, 6 axes) est disponible via "
        "`.claude/skills/eof/scripts/generate_radar_svg.py`.\n"
    )
    out.append("### Comment ce référentiel se lit\n")
    out.append(
        "- **Piliers 1 à 5 (Produit, Architecture, Infrastructure, Stockage, "
        ...
        "cochant la case correspondante. Score brut observé sur le Sheet "
        ...
        "- **Pilier 6 (Facilité de changement)** : dropdown **différent** des 5 "
        "autres piliers (confirmé par l'onglet \"À lire\" du Sheet source) : "
        ...
        "supposition par analogie avec les autres piliers, **non vérifiée** sur "
        "une donnée réelle.\n"
        "- **Score d'un pilier** = `Σ (score du critère × poids du critère)` "
        "÷ `Σ (poids de TOUS les critères du pilier, répondus ou non)`. "
        "**Un critère non répondu compte 0 au numérateur mais son poids reste "
        "au dénominateur** — il ne pénalise donc jamais le score, et un pilier "
        "vierge affiche 0 % de potentiel d'optimisation, pas \"inconnu\". "
        "C'est un biais méthodologique du Sheet source, pas une correction "
        "faite ici.\n"
        "- **Poids \"Sans objet\"** : l'onglet \"À lire\" indique qu'on peut "
        "exclure complètement un critère en réglant son poids sur \"Sans "
        "objet\" (pas juste répondre \"Non applicable\" à l'évaluation, qui "
        "elle laisse le poids compter au dénominateur). Aucun exemple réel de "
        ...
        "1 à 5 par critère (`(5 − réponse) ÷ 4 × poids`), ne contribue pas à ce "
        "tableau de synthèse.\n"
        "- **Poids** : chaque critère porte son propre poids (visible dans le "
        "Sheet source, colonne poids), différent d'un critère à l'autre — ce "
        "n'est jamais 1 partout.\n"
    )
    return "\n".join(out)
